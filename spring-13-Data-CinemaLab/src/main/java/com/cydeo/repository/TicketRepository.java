...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    List<Ticket>findTicketByDateTimeBetween(LocalDateTime date1,LocalDateTime date2);
    ...
    List<Ticket> allTicketsBoughtByUser( Long id);


    //Write a JPQL query that returns all tickets between a range of dates
    @Query("SELECT t FROM Ticket t WHERE t.dateTime BETWEEN ?1 and ?2")
    List<Ticket>allTicketsBetweenDates(LocalDateTime date1,LocalDateTime date2);
    ...
    Integer countNumberOfTicketsSpecificDates(Long id,LocalDateTime date1,LocalDateTime date2);


    //Write a native query to distinct all tickets by movie name
    @Query(value = "select distinct (m.name) FROM Ticket t JOIN movie_cinema c ON t.movie_cinema_id=c.id JOIN Movie m ON c.movie_id=m.id WHERE m.name",nativeQuery = true)
    List<Ticket>distinctAllTicketsByName();
